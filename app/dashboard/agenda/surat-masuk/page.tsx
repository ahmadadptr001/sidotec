...
const dataBreadcrumbs = [
  ...
  { name: "agenda", to: "#" },
  { name: "surat masuk", to: "/dashboard/agenda/surat-masuk" },
];

export default function HalamanAgendaSuratMasuk() {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: `Agenda_Surat_Masuk_${dariTanggal ? format(dariTanggal, "yyyy-MM-dd") : ""}`,
  });
  ...
  const handleTampilkanData = async () => {
    ...
    setHasFetched(true);
    try {
      const response = await rentangSurat(dariTanggal, sampaiTanggal, "masuk");
  ...
  return (
    <main className="min-h-screen bg-slate-50/50 pb-10 px-4 md:px-0">
      ...
      <div className="mt-8 bg-white print:p-0 hidden">
        <div
          ref={componentRef}
          className="p-0 md:p-8 print:p-4 mx-auto max-w-4xl"
        >
          {data.map((item, index) => (
            <div
              key={item.id}
            ...
            >
              ...
              <div className="border-x border-b border-black text-[12px]">
                ...
                {/* Baris 4 */}
                <div className="p-2 border-b border-black flex">
                  <span className="w-32">Asal Surat</span>
